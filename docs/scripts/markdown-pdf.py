...
# HTML comments (e.g. commented-out rough notes in the document preamble)
# must never be mistaken for real body content when locating the title or
# splitting the preamble from the body (see extract_preamble_and_body
# below). Both use the shared _markdown_shared module so the two never
# independently disagree about what counts as "real" content.
_TITLE_LINE_RE = re.compile(r"# (.+)")


def _find_title_match(md_text: str) -> re.Match | None:
    """Return the match for the first real (non-commented, non-fenced)
    ``# `` heading, or ``None`` if there isn't one.

    The match's span indexes correctly into the ORIGINAL ``md_text``:
    callers can use it both to read the title text (``extract_title``)
    and to excise the exact title line from the source (``main``),
    without those two users ever disagreeing about which line is "the
    title".
    """
    return _markdown_shared.find_first_real_match(md_text, _TITLE_LINE_RE)


def extract_title(md_text: str) -> str:
    """Extract the first real # heading as the document title.
    ...
    * the title-page ``<h1>`` (rendered via Jinja ``{{ title }}`` with
    ...
    "First real heading" skips any ``# ...`` line inside an HTML
    comment (see ``_find_title_match``) — otherwise commented-out
    scratch content above the real title (e.g. rough notes) is
    mistaken for it.
...
def strip_toc_block(md_text: str) -> str:
    ...
    Uses the shared ``_markdown_shared.TOC_START``/``TOC_END`` constants
    — markdown-toc.py generates this same block and previously this
    function retyped the identical literal strings independently.
    """
    before, after, found = _markdown_shared.strip_marked_block(
        md_text, _markdown_shared.TOC_START, _markdown_shared.TOC_END
    )
    if not found:
        return md_text
...
def extract_preamble_and_body(md_text: str) -> tuple[str, str]:
    """Split the document into preamble (before first real ##) and body
    (from first real ## onwards).

    The preamble includes the title and any introductory text. The body
    starts at the first real ## heading — one that is not inside a fenced
    code block or an HTML comment (see ``_markdown_shared``). Splitting
    at a commented-out ``## `` line (e.g. inside rough notes left in the
    preamble) would sever the comment mid-way: its closing ``-->`` would
    end up on the wrong side of the split, so the orphaned half is no
    longer recognised as a comment and renders as literal content.
    """
    m = _markdown_shared.find_first_real_match(md_text, _H2_LINE_RE)
    if m:
...
def main():
    ...
    # Recomputed against the current md_text (post strip_toc_block) via
    # the SAME comment-aware match extract_title() relied on above, so a
    # commented-out "# ..." heading earlier in the document (e.g. rough
    # notes) is left untouched — only the real title line, wherever it
    # falls in the file, is excised. A plain
    # `re.sub(r"^# .+\n*", "", md_text, count=1)` previously stood here
    # without re.MULTILINE, so `^` only matched the very start of the
    # string; once anything (a blank line, a comment) preceded the
    # title, the substitution silently did nothing and the real title
    # line was left behind to render again in the body.
    title_match = _find_title_match(md_text)
    if title_match:
        end = title_match.end()
        while end < len(md_text) and md_text[end] == "\n":
            end += 1
        md_text = md_text[: title_match.start()] + md_text[end:]
