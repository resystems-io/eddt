...
- [ ] **V4: Edge-Case and Coverage Tests:** Integration tests for specific scenarios not covered by V2's primary round-trip tests.
  - [ ] **V4a: BinaryUnmarshaler Round-Trip:** Add an integration test exercising `encoding.BinaryUnmarshaler` (not just `TextUnmarshaler`). Define a type implementing `MarshalBinary`/`UnmarshalBinary`, verify writer→reader round-trip including error-on-invalid-payload and zero-on-error semantics.
  - [ ] **V4b: Init-Time Error Paths (R5 negative):** Add integration tests that manually build Arrow records with incompatible column types (e.g., `*array.Int32` where `*array.String` is expected) and verify `NewXxxArrowReader` returns a descriptive error. Also test the dictionary-values type mismatch path (e.g., dictionary wrapping `*array.Int32` for a string column).
  - [ ] **V4c: Embedded Struct Flattening (R13):** Add an integration test with embedded (anonymous) struct fields. Verify the reader correctly maps flattened top-level Arrow columns back into the embedded struct hierarchy. Include a case with multiple levels of embedding.
  - [ ] **V4d: Missing Columns (R11/E5):** Add an integration test that builds an Arrow record missing one or more columns present in the Go struct. Verify `NewXxxArrowReader` succeeds and `LoadRow` leaves the corresponding Go fields untouched (at their zero or previous value).
