...
| 2026-03-17 | N3   | Added map reading support to reader-gen template. Six new sub-templates: `colFieldMap` (recursive struct field generation with `Items` suffix for map depth, delegating to `colFieldList` for list-valued maps), `initFieldMap`/`initFieldMapChild` (top-level schema lookup + recursive `Keys()`/`Items()` downcast), `loadFieldMap`/`loadFieldMapInner` (recursive extraction with `ValueOffsets`, `clear(m)` for R6 reuse, depth-variable naming `s0/e0/n0/j0/k0`). Cross-nesting: map→list delegates to `loadFieldListInner`/`initFieldListChild`. Supports simple maps, int keys, nested maps (`map[K]map[K]V`), map-with-list-values (`map[K][]V`), and named map types (`type Settings map[K]V`). Added `TestGenerator_ParseMapFields` (4 map field variants). Added 5 integration tests: `map-round-trip`, `map-int-keys-round-trip`, `nested-map-round-trip`, `map-list-values-round-trip`, `named-map-round-trip`. Full test suite verified (reader-gen, gencommon, writer-gen). |
| 2026-03-16 | N2   | Added fixed-size array support to reader-gen template. Widened `colField`/`colFieldList`/`initField`/`initFieldListChild` guards to dispatch `IsFixedSizeList` alongside `IsList` (shared Arrow column/init structure). Added two new load sub-templates: `loadFieldFixedList` (top-level nil guard + null→zero value instead of nil) and `loadFieldFixedListInner` (extraction loop with compile-time `FixedSizeLen` bound, discards end offset). Added cross-nesting branches in both `loadFieldListInner` (list→fixed) and `loadFieldFixedListInner` (fixed→list) for mixed types like `[][3]int32` and `[3][]int32`. Added `TestGenerator_ParseFixedSizeListFields` (3 fields: `[4]byte`, `[3]int32`, `[3][2]int32`). Added `fixed-size-round-trip` (basic + dirty-read overwrite), `nested-fixed-size-round-trip` (`[2][3]int32`), and `mixed-list-fixed-round-trip` (`[][3]int32` with null/empty list semantics) integration tests. Full test suite verified (reader-gen, gencommon, writer-gen). |
| 2026-03-17 | N4   | Added nested struct reading support via dual-constructor pattern: each struct gets `New{Name}ArrowReader(rec)` (record-level) and `new{Name}ArrowReaderFromStruct(col *array.Struct)` (nested, unexported). The struct-level constructor uses `dt.FieldIdx`/`col.Field` instead of `schema.FieldIndices`/`rec.Column`, but delegates to the same child init templates (`initFieldMapChild`, `initFieldListChild`). New `initStructField` template mirrors `initField` branch structure for struct-level init. Added struct branches to 8 existing templates: `colField`, `colFieldList`, `colFieldMap`, `initField`, `initFieldListChild`, `initFieldMapChild`, `loadField` (value + pointer), `loadFieldListInner`, `loadFieldMapInner`, `loadFieldFixedListInner`. Pointer-to-struct uses R6 allocation reuse and R7 null→nil. Map struct values use a temporary variable to avoid unaddressable map value issue. Added `lowerFirst` FuncMap helper. Added `TestGenerator_ParseStructFields` unit test. Added 6 integration tests: `struct-round-trip`, `pointer-to-struct-round-trip`, `nested-struct-round-trip`, `list-of-structs-round-trip`, `map-struct-values-round-trip`, `struct-with-containers-round-trip`. Full test suite verified (reader-gen, gencommon, writer-gen). |
