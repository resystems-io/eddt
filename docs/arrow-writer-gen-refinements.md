...
- [1. Audit Summary](#1-audit-summary)
  - [1.1 Supported Types (as of 2026-03-14)](#11-supported-types-as-of-2026-03-14)
  - [1.2 Bugs — Generate Broken (Non-Compiling) Code](#12-bugs-generate-broken-non-compiling-code)
  - [1.3 Missing Primitive — Easy Win](#13-missing-primitive-easy-win)
  - [1.4 Structural Gaps](#14-structural-gaps)
  - [1.5 Package Input — Go Import Paths vs Local Directories](#15-package-input-go-import-paths-vs-local-directories)
  - [1.6 Debatable / Low Priority](#16-debatable-low-priority)
- [2. Refinement Checklist](#2-refinement-checklist)
  - [Priority 1 — Fix Broken Code Generation](#priority-1-fix-broken-code-generation)
  - [Priority 2 — Easy Wins](#priority-2-easy-wins)
  - [Priority 3 — Structural Enhancements](#priority-3-structural-enhancements)
  - [Priority 4 — Debatable / Future](#priority-4-debatable-future)
...
| S7 | Pointer-wrapped compound types (`*[]T`, `*map[K]V`, `**T`)          | `fieldInfoFromExpr` handles `*ast.StarExpr` only when the inner expression is `Ident`, `SelectorExpr`, or a generic instantiation. Fields typed `*[]T` (pointer to slice), `*map[K]V` (pointer to map), or `**T` (pointer to pointer) fall through to `return FieldInfo{}, fmt.Errorf("unsupported pointer type")`. These are valid Go field shapes — e.g. an optional list field where nil means absent and a pointer to an empty slice means explicitly set to empty. Arrow encoding: a nullable outer pointer wraps the inner type's Arrow representation; null when the outer pointer is nil, inner value otherwise. Fix: add `*ast.ArrayType`, `*ast.MapType`, and `*ast.StarExpr` inner-expression branches to the `*ast.StarExpr` handler in `fieldInfoFromExpr`. Writer template: outer nil → `AppendNull()`; outer non-nil → unwrap and delegate to the existing `IsList` / `IsMap` / `IsPointer` append branches. |
| S8 | Grouped field declarations (`A, B T` — several names, one type)     | ~~`gencommon` resolves struct fields from the AST but read only `field.Names[0]` in both the struct-field loop and the embedded-field promotion loop, so a declaration like `Start, End uint64` emitted only its first name — silently dropping the rest from the Arrow schema and Parquet. `runtime.SequenceRange.End` was lost this way, breaking the `Header.Provenance.Gaps` round-trip.~~ **Fixed (2026-06-18).** Both loops now iterate `field.Names`. Shared `gencommon` fix — applies to arrow-writer-gen and arrow-reader-gen alike; guarded by `TestGroupedFieldDeclRoundTrip` in the reader-gen integration tests.                                                                                                                                                                                                                                                                                               |
...
- [x] **S7: Support pointer-wrapped compound types (`*[]T`, `*map[K]V`, `**T`)** *(2026-05-26)* —
  `fieldInfoFromExpr` (`internal/arrow/gencommon/resolve.go`) rejects these field
  shapes with `"unsupported pointer type"` because the `*ast.StarExpr` handler only
  dispatches on `Ident`, `SelectorExpr`, and generic instantiation inner expressions.
  Arrow encoding: a nullable outer pointer wraps the inner type's Arrow
  representation — null when the outer pointer is nil, the inner value otherwise.
  Three new inner-expression branches needed in the `*ast.StarExpr` handler:
  - `*ast.ArrayType` inner → `IsPointer=true, IsList=true` (nullable list; `*[]byte`
    treated as nullable Binary, not nullable List of Uint8, consistent with
    the existing `[]byte` → Binary special case).
  - `*ast.MapType` inner → `IsPointer=true, IsMap=true` (nullable map).
  - `*ast.StarExpr` inner → `IsPointer=true` with recursive inner `FieldInfo`
    resolution (nullable pointer-to-pointer, Arrow-encoded as nullable scalar or struct).
  Writer template change: when `IsPointer=true` and the inner type is a compound
  (`IsList`, `IsMap`, or another pointer), emit a null check; on nil, call
  `AppendNull()` on the outer builder; on non-nil, dereference and delegate to the
  existing `IsList` / `IsMap` / `IsPointer` append branches.
  - Files: `gencommon/resolve.go`, `gencommon/resolve_test.go`,
    `writer-gen/template.go`, `writer-gen/integration_test.go`
...
| 2026-05-26 | S7   | Pointer-wrapped compound types: `*[]T` (nullable list), `*map[K]V` (nullable map), `**T` (outer-nullable scalar pointer). Three new inner-expression branches in `fieldInfoFromExpr` `*ast.StarExpr` handler; range-dereference fixes in writer template; `**T` branch before scalar `else`. Unsupported shapes (`*[N]T`, `**struct`) warn and skip. |
| 2026-06-18 | S8   | Grouped field declarations (`A, B T`) emit every name; `gencommon` field-resolution loops iterate `field.Names` (was `Names[0]`). Restores fields dropped from the schema/Parquet (e.g. `SequenceRange.End`).                                                                                                                                        |
