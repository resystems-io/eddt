...
| S6 | Generic struct instantiations silently skipped                      | ~~`fieldInfoFromExpr` has no `*ast.IndexExpr` / `*ast.IndexListExpr` branch, so fields like `FieldDelta[int32]` are skipped with a warning. Generic instances need a derived Go identifier (`FieldDelta_Int32`), type-checker-driven field expansion, and a `GoTypeExpr` that renders the correct qualified instantiation in generated code.~~ **Fixed (2026-05-15).** Added `*ast.IndexExpr`/`*ast.IndexListExpr` handling, `derivedInstanceName`, `fieldInfoFromGenericInstantiation`, `StructInfo.GoTypeExpr`, and `StructInfo.instantiatedType`. Writer template uses `GoTypeExpr` for `Append`/`AppendXxxStruct` parameter types. |
...
- [x] **S6: Generic struct instantiation support** *(2026-05-15)* — Fields whose
  types are generic struct instantiations (e.g. `FieldDelta[int32]`, `FieldDelta[*Inner]`)
  now parse and generate correctly. `fieldInfoFromExpr` handles `*ast.IndexExpr` and
  `*ast.IndexListExpr`; `derivedInstanceName` produces a valid Go identifier
  (`FieldDelta_Int32`); `fieldInfoFromGenericInstantiation` queues the instantiation
  for type-checker-driven field expansion. `StructInfo.GoTypeExpr` carries the
  qualified instantiation string (`FieldDelta[int32]`); writer template uses it for
  `Append`/`AppendXxxStruct` parameter types.
  - Files: `gencommon/gencommon.go`, `gencommon/resolve.go`, `gencommon/generics_test.go`,
    `writer-gen/template.go`, `writer-gen/integration_test.go`
...
| 2026-05-15 | S6   | Generic struct instantiation support: `*ast.IndexExpr` handling, `derivedInstanceName`, `GoTypeExpr`, named-type-over-primitive fix in `fieldInfoFromType` |
