...
- [x] **C-09: Cross-generator Header/Provenance envelope round-trip.**
  *(2026-06-18)* Prove the full `runtime.Header` envelope — `EntityID`, `ChainID`,
  `Previous`/`NextChainID`, `Closed`, `Sequence`, `EffectiveAt`/`PublishedAt`, and
  `Provenance[]{Metadata, Gaps}` — embedded in a generated Delta round-trips
  losslessly through arrow-writer-gen → Parquet → arrow-reader-gen, verifying
  R-DG-052 at the envelope level (Provenance shape R-CL-004; accumulation
  R-DG-032 / R-CL-018). Closes the gap where C-06 / C-08 asserted only
  `Header.ChainID` + `Sequence`. A schema-presence guard runs before the value
  assertions to defeat arrow-writer-gen's silent-skip path. Surfaced a gencommon
  defect — grouped field declarations (`Start, End uint64`) emitted only the first
  name, dropping `SequenceRange.End` — fixed separately (commit prefixed `arrow:`,
  guarded by `TestGroupedFieldDeclRoundTrip`).
  - Files: `internal/deltagen/integration_arrow_header_test.go` (new),
    `internal/deltagen/testdata/arrow_roundtrip/snapshot_header.go` (new),
    `internal/deltagen/integration_arrow_test.go` (HeaderProvenance subtest).
  - Tests: `TestIntegration_ArrowRoundTrip/HeaderProvenance` PASS (Arrow reader +
    DuckDB nested Provenance / Gaps / Metadata + EntityID / timestamp / anchor
    checks + Arrow schema-presence).
...
| 2026-06-18 | C-09                     | Full Header/Provenance envelope Parquet round-trip (R-DG-052): EntityID, anchor pointers, bitemporal times, Provenance{Metadata, Gaps}. Surfaced + fixed a gencommon grouped-field-declaration bug (SequenceRange.End dropped); new TestGroupedFieldDeclRoundTrip guard (committed under `arrow:`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
