...
- [ ] **N-01: Compositional emission for `delta.nested` on struct
  value (`X T` → `X TDelta`).** Recursive parse + emit; generate
  companion Delta type and `Apply` / `Diff` (no `Coalesce`) per
  delta-gen §4.3.
  - Files: `internal/deltagen/parse.go`,
