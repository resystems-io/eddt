// Package struct_key is the R-DG-019 struct-valued entity key corpus fixture.
//
// It covers the multi-field EntityID hash path (R-DG-034) with a struct-valued
...
// (R-DG-034 stability invariant) and emits an EntityID function and method wrapper.
//
// Coverage:
//   - Struct-valued entity.key (SessionKey{TenantID string, SessionN uint64})
//   - Multi-field EntityID hash with lexicographic field-name order (R-DG-034)
...
// hashes them in lexicographic field-name order (R-DG-034):
...
// SessionSnapshot covers the struct-key EntityID hash path (R-DG-034).
//
// Delta encoding axes covered:
//   - Key:   struct-valued entity.key → EntityID hashes SessionKey fields (R-DG-034)
