// Package baseline is the C-01 primary conformance corpus fixture.
//
...
// is the canonical input for C-02 / C-03 / C-04 property-based tests.
//
// Coverage:
...
//   - Scalar string entity.key (single-field EntityID hash path, EM-05).
package baseline
...
//   - Tags:     ShapeSlice (atomic per E-15) → SetTags *[]string in TDelta
//   - Attrs:    ShapeMap (atomic per E-16)   → SetAttrs *map[string]string in TDelta
...
type BaselineSnapshot struct {
	...
	Meta MetaInfo
	// Tags is a string-slice atomic field (ShapeSlice, atomic per E-15).
	// Use delta.nested to opt into set-diff encoding (N-04).
	Tags []string
	// Attrs is a string-keyed string-valued map atomic field (ShapeMap, atomic per E-16).
	// Use delta.nested to opt into element-wise encoding (N-03).
