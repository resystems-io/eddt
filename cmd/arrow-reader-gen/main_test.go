...
func TestCLI_GenerateReaders(t *testing.T) {
	tmpDir := t.TempDir()
...
`
	...
	outFile := filepath.Join(tmpDir, "arrow-reader-gen.go")
	...
	got := string(out)
	if !strings.Contains(got, "type PersonArrowReader struct") {
		t.Errorf("Expected PersonArrowReader, but got:\n%s", got)
	}
}
...
`
	...
	if !strings.Contains(got, "func (r *PersonArrowReader) LoadRow(i int, out *mydummy.Person)") {
		t.Errorf("Expected func (r *PersonArrowReader) LoadRow(i int, out *mydummy.Person), but got:\n%s", got)
	}
}
